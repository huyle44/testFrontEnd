...
import React, { useState, useRef, useEffect } from 'react';
// (SỬA) Thay đổi đường dẫn import
import { recognition } from '../utils/chatHelpers';

// (SỬA) Xóa 'handleSend' và 'stagedFiles' khỏi tham số
export const useSpeech = () => {
  ...
  // (SỬA) Xóa bỏ toàn bộ useEffect của SpeechRecognition.
  // Nó sẽ được chuyển về component cha (InterviewChatbot.jsx).
  ...
  const handleVoiceToggle = () => {
    ...
    // (SỬA) Chỉ thay đổi state, không gọi start/stop
    setIsRecording(!isRecording); 
    return true; // (SỬA) Trả về true nếu thành công
  };
